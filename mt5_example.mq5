...
// - list all pending orders place by this client   [#3]
...
input string            i_fa_alloc_type                     = "";   // alloc type (FA only):  can be 'account','group' or 'profile'
input string            i_fa_alloc_name                     = "";     // alloc name (FA only):  name of account,group or profile;Group can have default 'All'
...
int OnInit()
{
    ...
    string config_name=StringFormat("mt_%s",TerminalInfoString(TERMINAL_NAME));
    ...
    if(__conid <= 0)
    {
     ...
     __id_allocation=ibgCREATE_ALLOCATION(__connection_id,i_fa_alloc_name,i_fa_alloc_name,i_fa_alloc_method, i_fa_percentage);
	...
	double ret_double=ibgACCOUNT_INFO (__connection_id,tcACCOUNT_EQUITY,"");   
